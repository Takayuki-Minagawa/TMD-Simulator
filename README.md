...
- 時刻歴応答（基礎入力）
- 時刻歴応答（強制力入力）
- 応答波形の表示切替（応答加速度 / 層間変位 / 層間速度）
- 変位アニメーション（各質点の時刻歴変位を建物形状で再生）
- 既存結果 CSV の再読込表示
- 結果 CSV 出力
- ワークスペース ZIP の入出力
...
### Ver.1.0.2 (2026-03-06)
**新機能:**
- 応答波形の表示切替機能（応答加速度 / 層間変位 / 層間速度をラジオボタンで選択）
- 変位アニメーション機能（基礎を原点として各質点の相対変位を建物形状で時刻歴再生）
  - 再生/一時停止、先頭戻し、タイムスライダー、再生速度（x0.25〜x4）
  - 最大変位が層間高さの1.5倍に収まるアスペクト比で描画

**改善:**
- 結果テーブルからTMD Max値（TMD Amax / TMD Dmax）を削除（主架構の応答に集中）
- TMD描画チェックボックスを削除
- 最上階の床レベル表示をRFに統一（チャート凡例・アニメーション・CSVヘッダー）
- 応答加速度チャートに全質点を凡例付きで表示（従来は最上階のみ）

### Ver.1.0.1 (2026-03-06)
**新機能:**
- 固有値解析に固有ベクトルテーブル（最大値1.0基準化）を追加
- 刺激関数モード図（全次数ポリライン表示）を追加
- 固有値解析結果のCSV出力機能（UTF-8 BOM付き、Excel対応）

**改善:**
- モード図のグラフ幅をスライダーで調整可能に
- モードラベルを「1次」「2次」表記に修正
...
- CSVパース時のエラーハンドリング強化
- モデルデータの検証機能追加
- モデル編集画面で層表示を大きい順に変更（上階から表示）
- 「Floor」表記を日本語「層」/ 英語「Layer」に統一
